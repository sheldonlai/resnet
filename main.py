...
tf.app.flags.DEFINE_boolean('core', True, """train only the core layers""")
...
tf.app.flags.DEFINE_integer('save', 500, """save every 100 iteration""")

data_loader = CifarDataLoader(augmentation=FLAGS.data_augmentation)

model = ResNetClassifier(data_loader=data_loader, all_layers=not FLAGS.core,
...
if FLAGS.train:
    ...
    while step <= 65000:
        ...
        summary, _, t_acc = sess.run([merged, model.train_op, model.accuracy],
                                     feed_dict={model.x: x, model.y: y, model.phase: 1})
        train_writer.add_summary(summary, step)
        step = tf.train.global_step(sess, model.global_step)
        train_accs.append(t_acc)

        if step == 32000 or step == 45000:
        ...
        if step % FLAGS.save == 0:
            ...
            print('step %d, valid accuracy %f%%, training_acc %f%%, training_loss %f, time elasped %s' %
                  (step, acc, t_acc, loss, time.time() - start_time))
            train_accs = []
